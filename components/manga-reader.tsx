...
// ── Adsterra native banner zone (loading screen + top banner) ─────────────────
// Single zone shared between loading and top-banner slots.
// Only one is ever in the DOM at a time — no container ID conflict.
const AD_ZONE = {
...
// ── End-of-chapter iframe banner (separate network / format) ──────────────────
// This is always in the DOM once panels finish loading — no slot conflict
// because it uses its own script + iframe, not the Adsterra container above.
const BOTTOM_BANNER = {
  atKey:    "4926ae8bee0daefb765d260889536807",
  src:      "https://www.highperformanceformat.com/4926ae8bee0daefb765d260889536807/invoke.js",
  height:   90,
  width:    728,
};

// ── BottomBannerAd component ──────────────────────────────────────────────────
// Injects the iframe banner script once on mount, cleans up on unmount.
function BottomBannerAd({ chapterKey }: { chapterKey: number | string }) {
  const injected = useRef(false);
  const containerId = `bottom-banner-container-${chapterKey}`;

  useEffect(() => {
    if (injected.current) return;
    injected.current = true;

    // Set atOptions on window before the script loads
    (window as any).atOptions = {
      key:    BOTTOM_BANNER.atKey,
      format: "iframe",
      height: BOTTOM_BANNER.height,
      width:  BOTTOM_BANNER.width,
      params: {},
    };

    const s = document.createElement("script");
    s.src = BOTTOM_BANNER.src;
    s.async = true;
    s.setAttribute("data-bottom-banner", containerId);
    document.body.appendChild(s);

    return () => {
      const el = document.querySelector(`script[data-bottom-banner="${containerId}"]`);
      if (el) el.remove();
  ...
  return (
    <div className="w-full flex flex-col items-center py-4 bg-slate-900/60 border-y border-slate-700/40">
      <p className="text-[10px] text-slate-600 uppercase tracking-widest mb-2 select-none">
        Advertisement
      </p>
      {/* The iframe banner script targets the page body — this div is just
          the visual container so the ad sits in the right place in the layout */}
      <div
        id={containerId}
        style={{ width: BOTTOM_BANNER.width, maxWidth: "100%", height: BOTTOM_BANNER.height }}
      />
    </div>
  );
}

// ── Ad slot type for the native banner (loading / top-banner only) ────────────
type AdSlot = "loading" | "top-banner" | "none";
...
export function MangaReader({
  ...
  // ── Switch slot cleanly: "none" → 100ms → new slot ───────────────────────
  ...
  // ── TRANSITION: loading → top-banner ─────────────────────────────────────
  // Fires when loading is done AND panel 1 enters the viewport.
  // This is the cleanest possible trigger: supabase data received,
  // loading UI is gone, user literally sees the first panel.
  useEffect(() => {
  ...
  // ── Shared props for the native banner ───────────────────────────────────
  const sharedAdProps = {
  ...
  const loadMorePanels = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      for (let i = 1; i <= 5 && currentLength + i <= totalPanelsToUse; i++) {
        nextPanels.push(currentLength + i);
      }
      if (nextPanels.length > 0) setDisplayedPanels((prev) => [...prev, ...nextPanels]);
      setIsLoading(false);
    }, 200);
        ...
        >
          ...
          >
            ...
            {/* ── SLOT 1: Loading screen ─────────────────────────────────────
                Native banner shown while supabase fetch is in progress.
                Unmounts the moment loading finishes and panels appear.
            ──────────────────────────────────────────────────────────────── */}
            ...
            {isLockedChapter ? (
              ...
              displayedPanels.length > 0 && (
                ...
                >
                  {/* ── SLOT 2: Top-of-chapter native banner ──────────────────
                      Fires when panel 1 enters the viewport via
                      IntersectionObserver (set up in useEffect above).
                      Scrolls away naturally — no dismiss needed.
                  ──────────────────────────────────────────────────────────── */}
                  ...
                  {displayedPanels.map((panelNumber) => (
                    <div
                      ...
                      ref={(el) => {
                        ...
                        // Panel 1 ref drives the top-banner viewport trigger
                        if (panelNumber === 1) firstPanelRef.current = el;
                      }}
                  ...
                  {/* ── End of chapter ─────────────────────────────────────────
                      BOTTOM BANNER — completely separate network (highperformanceformat).
                      Uses its own script + iframe so zero conflict with the
                      Adsterra native banner above.
                      key=chapter forces full remount on chapter navigation.
                  ──────────────────────────────────────────────────────────── */}
                  {allPanelsLoaded && (
                    <div className="text-center py-8 space-y-4">
                      <p className="text-slate-400 text-sm">End of chapter</p>

                      <BottomBannerAd key={chapter} chapterKey={chapter} />
